...
class IntervalManager:
    ...
    def add_ignore_share(self, itvl):
        ...
        if low_interval_index == high_interval_index == -1:
        ...
        elif len(self.interval) == 1 and itvl.low < self.interval[0].high and itvl.low > self.interval[0].low and \
                self.interval[0].type != 0:
            self.interval.append(itvl)
        elif len(self.interval) == 1 and itvl.high > self.interval[0].low and itvl.high < self.interval[0].high and \
                self.interval[0].type != 0:
            self.interval.insert(0, itvl)
        elif high_interval_index == low_interval_index and len(self.interval) == 1 and (
                itvl.high > self.interval[0].high or itvl.low < self.interval[0].low or (
                high_interval_index == -1 or low_interval_index == -1) and len(self.interval) == 1 and (
                        itvl.high > self.interval[0].high or itvl.low < self.interval[0].low)):
            ...
            highItvl = Interval(itvl.high, self.interval[high_interval_index].high,
                                self.interval[high_interval_index].type)
    ...
    def add_interval(self, itvl):
        ...
        if (low_interval_index == -1 or high_interval_index == -1):
            ...
            high_itvl = Interval(itvl.high, self.interval[high_interval_index].high,
                                 self.interval[high_interval_index].type)
...
class MuffinsAuto:
    ...
    def three_share_test(self):
        ...
        if is_less_one or is_less_two:
            raise ThreeShareException(
                "DARN, THIS IS THE CASE BILL WARNED ME ABOUT WHERE THERE ARE THREE NUMBERS-OF-SHARES.")
    ...
    def find_upper_range(self):
        ...
        res = Interval(new_bound, self.subtract_from(self.alpha, Fraction(1)).numerator, 1,
                       self.num_low_share * self.low_share)
        return res
    ...
    def find_lower_range(self):
        ...
        self.manager.add_ignore_share(
            Interval(self.alpha.numerator, new_bound, 2, self.num_high_share * self.high_share))
    ...
    def find_blocks(self):
        ...
        for elem in temp_list:
            self.manager.add_interval(
                Interval(self.alpha.denominator - elem.high, self.alpha.denominator - elem.low, 0))
    ...
    def refine(self, lst, int_type):
        ...
        for elem in to_be_added:
            ...
            index_itvl_buddy = self.manager.find_interval(
                Interval(self.share.denominator - elem.high, self.share.denominator - elem.low, 0))

            if self.manager.get(index_itvl + 1).type == 1 and self.low_share == 2 or self.manager.get(
                    index_itvl - 1).type == 1 and self.low_share == 2:
                self.match_gap(self.manager.get(index_itvl))
            if self.manager.get(index_itvl_buddy + 1).type == 1 and self.low_share == 2 or self.manager.get(
                    index_itvl_buddy - 1).type == 1 and self.low_share == 2:
    ...
    def buddy_one(self, itvl):
        ...
        for j in range(len(self.manager.get_all())):
            if self.manager.get(j).has_bound(self.share.denominator - itvl.low) and self.manager.get(j).has_bound(
                    self.share.denominator - itvl.high):
                has_buddy = True
        ...
        if not has_buddy:
            ...
            self.manager.add_interval(
                Interval(self.share.denominator - itvl.high, self.share.denominator - itvl.low, itvl.type,
                         itvl.num_shares))
    ...
    def max_or_min(self, s, int_type, focus):
        ...
        if abs(dmax - self.share.numerator) > abs(dmin - self.share.numerator) and itvl.contains_bound(
                self.minimize(s, focus, new_type)):
            return 1
        else:
            return 2
    ...
    def find_buddy_match_definitions(self, full_list, type_list, itvl):
        ...
        match_of_buddy_index = self.manager.find_interval(
            Interval(self.share.numerator - buddy_in_match.high, self.share.numerator - buddy_in_match.low,
                     buddy_in_match.type))
        ...
        if d_type == 2:
            ...
            for i in range(1, len(lst) + 1):
                ...
                if i == highest:
                    ...
                    s = (self.num_low_share * self.low_share) + (self.num_high_share * self.high_share) - (
                                2 * known_share_num)
                    print("J_{}| = {}".format(f, s))
                elif i == highest_lone:
                    print("|", end="")
            ...
            for i in range(1, len(lst) + 1):
                ...
                if i == highest:
                    ...
                    print("I_" + str(union_list[len(union_list) - 1]) + "| = " + str(
                        (self.num_low_share * self.low_share) + (self.num_high_share * self.high_share) - (
                                    2 * known_share_num)))
                elif i == highest_lone:
                    print("|", end="")
                    for j in range(len(lone_share_list) - 1):
    ...
    def find_initial_equations2(self, int_type):
        ...
        for i in range(len(self.manager.get_all_not(0))):
            ...
            if index != -1 and self.find_buddy_definitions(self.manager.get_all_not(0),
                                                           self.manager.get_all_not(0)[index]) < index and (
                    not self.manager.get_all_not(0)[i] in self.manager.get_all_type(int_type) or not
            self.manager.get_all_not(0)[index] in self.manager.get_all_type(int_type)):
                list3 = []
                list4 = []
    ...
    def shift_one_side(self, initial_list):
        for elem in initial_list:
            if (len(elem.arg1) == 0 and type(elem.arg2[0].variable) == Student) or type(
                    elem.arg1[0].variable) == Student and type(elem.arg2[0].variable) == Student:
                counter = 0
    ...
    def match(self):
        ...
        for i in range(len(self.manager.get_all())):
            ...
            if self.low_share == 2 and self.manager.get(i).type == 1 or self.high_share == 2 and self.manager.get(
                    i).type == 2:
                if self.share.numerator - self.manager.get(i).low == self.manager.get(i).high:
                    ...
                    index = self.manager.find_interval(
                        Interval(self.share.denominator - midpoint, self.share.denominator - low, 2,
                                 self.myint(new_num_shares / 2)))
                    self.manager.get(index - 1).num_shares = self.myint(new_num_shares / 2)
        ...
        if size == 2:
            if self.manager.get(size - 1).low >= midpoint:
                if self.manager.get(size - 1).type == 2 and self.high_share * self.num_high_share > self.m:
                    ...
                    print(
                        "Contradiction. " + str(self.high_share) + " * s" + str(self.high_share) + " > m since " + str(
                            self.high_share) + " * " + str(self.num_high_share) + " > " + str(self.m))
                    ...
                    print("Contradiction. " + str(self.low_share) + " * s" + str(self.low_share) + " > m since " + str(
                        self.low_share) + " * " + str(self.num_low_share) + " > " + str(self.m))
    ...
    def main(self):
        try:
            ...
            if len(self.manager.get_all()) == 2 and self.manager.get(1).type == 0 and itvl.low <= self.manager.get(
                    1).low and itvl.high >= self.manager.get(1).high:
            ...
            try:
                if not self.is_bill():
                    if (len(self.manager.get_all()) == 2 and self.manager.get(0).high > self.manager.get(1).low):
                        print("Tried f(" + str(self.m) + "," + str(self.s) + "\\le " + str(
                            self.alpha) + " and failed BOO-overlapping intervals", end="")
                    else:
                        self.mid_point()
                        ...
                        if num_solutions == 1:
                            ...
                            if num_high_interval > 1 and num_low_interval > num_high_interval or num_low_interval > 1 and num_high_interval > num_low_interval:
                                ...
                                if num_sol != 1:
                                    print()
                                    print("Tried f(%s, %s) \\le %s and succeeded YEAH-BILL2" % (
                                    self.m, self.s, self.alpha), end="")
                            else:
                                print(
                                    "Tried f(%s, %s) \\le %s and failed BOO-ERIK FAILED" % (self.m, self.s, self.alpha),
                                    end="")
                            ...
                            if num_high_interval > 1 and num_low_interval > num_high_interval or num_low_interval > 1 and num_high_interval > num_low_interval:
                                ...
                                if num_sol != 1:
                                    print(
                                        "WARNING: Solved by bill2 though ERIK did not work for the first set of shares",
                                        end="")

                            else:
                                print(
                                    "Bill 2 was attempted because there were 0 students that used the first number-of-share and subsequently failed.",
                                    end="")
                                print()
                else:
