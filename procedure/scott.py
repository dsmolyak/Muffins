...
def scott(muffins, majors, minors):
    ...
    if Ns2 == 0:  # must always be that Nm*Vm = Ns1*Vs1, and Nm*Pm = Ns1*Ps1, so Vm/Pm = Vs1/Ps1
        ...
        majorPieces = [[Vs1 / Ps1 for i in range(int(Ps1))] for j in
                       range(int(Ns1))]  # All major pieces of size Vs1 / Ps1
        ...
        (subMuffinPieces, subMajorPieces, subMinorPieces) = scott(minors, newMajors,
                                                                  newMinors)  # run the subproblem, next we need to reconstruct solution
        ...
        deconstructedLChains = [deconstructLchain(pieces, Pm, Ps2, Vm, Vs2, L) for pieces in
                                subMajorPieces]  # L-chains became the sub-majors
        deconstructedLm1Chains = [deconstructLchain(pieces, Pm, Ps2, Vm, Vs2, L - 1) for pieces in
                                  subMinorPieces]  # (L-1)-chains became the sub-minors
        muffinPieces = sum([dchain[0] for dchain in deconstructedLChains + deconstructedLm1Chains],
                           [])  # get first elements out of tuples, so we have lists of students
...
def deconstruct1chain(pieces, Pm, Ps2, Vm):
    ...
    muffinsMissingLastPiece = divide_chunks(pieces,
                                            Pm - 1)  # first find list of most of muffins, missing last piece (connected to student)
    muffins = [partialM + [Vm - sum(partialM)] for partialM in
               muffinsMissingLastPiece]  # find what the size of the last piece is
...
def deconstructLchain(pieces, Pm, Ps2, Vm, Vs2, L):
    ...
    #    return deconstruct1chain(pieces, Pm, Ps2, Vm)
        ...
        bridgePieces = pieces[
                       numLeftPieces:numLeftPieces + Pm - 2]  # Pieces that go to muffin connecting left student to next student
        leftOverPieces = pieces[numLeftPieces + Pm - 2:]  # all the rest of the pieces

        muffinsMissingLastPiece = divide_chunks(leftPieces,
                                                Pm - 1)  # first find list of most of muffins, missing last piece (connected to student)
        muffins = [partialM + [Vm - sum(partialM)] for partialM in
                   muffinsMissingLastPiece]  # find what the size of the last piece is
        ...
        newPieces = [student[
                         -1]] + bridgePieces + leftOverPieces  # add that last piece of the student on to the pieces to be used in recursion - this is bridge muffin piece
