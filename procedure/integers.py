...
def solve(m, s, d=None, intervals=None, pieces=None, spew=False, returnMatrix=False):
    ...
    if d == None:
        d = lcm([fraction(f)[1] for f in
                 points])  # denominator that we are going to use is lcm of denominators on ends of intervals
...
def posIntSolve(numbers, addTo):
    ...
    def posIntSolveImpl(numbers, addTo, numsSoFar):
        ...
        for value in numbers:  # try adding each number
            ...
            if sumn == addTo:
                ...
                lowerNumbers = [(num, i) for (num, i) in numbers if
                                num <= n]  # wlog, pieces decrease in size from first to last
    ...
    indices = [[info for (n, info) in possibility] for possibility in
               results]  # this is a list of lists of indices of numbers used
