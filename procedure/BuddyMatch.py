...
def findCutPoints(eVals, intervals, a, d, k):
    ...
    for index in eVals:
        ...
        if allowedXValues == sympy.EmptySet() or allowedXValues.end <= 0:  # 2*a/5:#if the constraint can never be met or is smaller than X can be
            ...
            cuts.append((allowedXValues.end,
                         eVar == 0))  # add on (min val of X where constraint can't be met, constraint that variable is zero)
    cuts = sorted(cuts)
    return (always, cuts)
...
def doFirstCase(a, d, k):
    ...
    values = [val / denom for val in
              [d * k + X, d * k + a / 2, d * k + a - X, d * k + 2 * X, d * k + 2 * X, d * k + (a + d) / 2,
               d * k + a + d - 2 * X]]
...
def doSecondCase(a, d, k):
    ...
    denom = 3 * d * k + a
    values = [d * k + X, d * k + a / 2, d * k + a - X, d * k + 2 * X, d * k + 2 * X, d * k + 2 * a - 2 * X,
              d * k + 3 * X, d * k + (a + d) / 2, d * k + a + d - 3 * X, d * k + a + d - 3 * X, d * k + d - a + 2 * X,
              d * k + a + d - 2 * X]
    vald = [val / denom for val in values]
    intervals = [(vald[0], vald[1]), (vald[1], vald[2]), (vald[4], vald[5]), (vald[6], vald[7]), (vald[7], vald[8]),
                 (vald[10], vald[11])]
...
def doThirdCase(a, d, k):
    ...
    denom = 3 * d * k + a
    values = [d * k + X, d * k + a / 2, d * k + a - X, d * k + 2 * X, d * k + 2 * X, d * k + a + d - 3 * X,
              d * k + d - a + 2 * X, d * k + d - a + 2 * X, d * k + (a + d) / 2, d * k + 2 * a - 2 * X, d * k + 3 * X,
              d * k + a + d - 2 * X]
    vald = [val / denom for val in values]
    intervals = [(vald[0], vald[1]), (vald[1], vald[2]), (vald[4], vald[5]), (vald[7], vald[8]), (vald[8], vald[9]),
                 (vald[10], vald[11])]
    totals = [a + d, a + d, y, 2 * d - a - y, 2 * d - a - y, y]
...
def f(m, s):
    ...
    d = m - s
    a = s % (3 * d)
    if a == 0 or 2 * d + 1 <= a <= 3 * d - 1:
        return Fraction(1, 3)
    k = (s - a) / (3 * d)
    res = solve(a, d, k)
    if res.__class__ == Fraction:
        return res
    else:
