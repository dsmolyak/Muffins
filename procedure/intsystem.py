...
def solve(m, minVal, maxVal):
    ...
    rows = m.tolist()
    numVars = m.cols - 1
    variables = [LpVariable('x' + str(i), minVal, maxVal, LpInteger) for i in
                 range(numVars)]  # make variables x0, x1, x2, ...
    ...
    for row in rows:  # make one constraint for each row in matrix
        ...
        dotProduct = [var * coeff for (var, coeff) in
                      zip(variables, varVector)]  # if coeff != 0] #list of coefficients * xi
